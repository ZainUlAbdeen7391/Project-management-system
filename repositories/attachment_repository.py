...
async def _check_entity_access(cur, entity_type: str, entity_id: int, user_id: int):
...
async def upload_attachment(cur, entity_type: str, entity_id: int, file: UploadFile, user_id: int):
    #check access
    ...
    if file_size>MAX_FILE_SIZE:
        raise ValueError("Upoad file under 10MB")
    # Build folder path: uploads/tasks/1/ or uploads/projects/2/
    ...
    await cur.execute(
        ...
        INSERT INTO tbl_attachments
            (entity_type, entity_id, file_name, file_size, file_type, file_path, uploaded_by, status)
    ...
    attachment_id = getattr(cur, "lastrowid", None)
    ...
    if not attachment_id:
        raise RuntimeError("Failed to retrieve attachment_id after insert")
...
async def list_attachments(cur, entity_type: str, entity_id: int, user_id: int):
    # Check access
    await _check_entity_access(cur, entity_type, entity_id, user_id)
...
async def delete_attachment(cur, attachment_id: int, user_id: int):
